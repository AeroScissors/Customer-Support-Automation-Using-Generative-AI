...
class TicketService:
    ...
    def create_ticket(
        ...
        # ✅ FIX: Include decision_reason in the returned TicketResponse
    ...
    def update_ticket_status(
        ...
        # ✅ FIX: Added HUMAN_RESOLVED to the set that gets resolved_at timestamp
        resolved_statuses = {
            TicketStatus.CLOSED,
            TicketStatus.AI_RESOLVED,
            TicketStatus.RESOLVED,
            TicketStatus.HUMAN_RESOLVED,
        }
    ...
    def append_message(self, ticket_id: str, message: dict) -> Optional[dict]:
        # ✅ FIX: Also update status to HUMAN_RESOLVED when agent sends a message
        result = self.collection.update_one(
            {"ticket_id": ticket_id},
            {
                "$push": {"messages": message},
                "$set": {
                    "status": TicketStatus.HUMAN_RESOLVED,
