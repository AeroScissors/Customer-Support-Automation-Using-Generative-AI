...
def normalize_ticket(ticket: dict) -> dict:
    """
    Ensure ticket always has `messages` and `decision_reason`.
    Backward-compatible with legacy tickets.
    """

    # ✅ FIX: Ensure decision_reason always exists
    if "decision_reason" not in ticket or not ticket["decision_reason"]:
        ticket["decision_reason"] = "No explanation provided"
